...
document.addEventListener('alpine:init', () => {
  Alpine.data('app', () => ({
    ...
    init() {
      ...
      // Verificar si las notificaciones están soportadas y cargar preferencia
      this.checkNotificationSupport();

      // Agregar listeners para detectar cuando la ventana está activa/inactiva
    ...
    initNotificationSound() {
      this.notificationSound = new Audio('https://assets.mixkit.co/sfx/preview/mixkit-simple-notification-tone-951.mp3');
    ...
    checkNotificationSupport() {
      ...
      // Si el usuario ya dio permiso anteriormente
    ...
    requestNotificationPermission() {
      ...
      Notification.requestPermission().then(permission => {
        if (permission === 'granted') {
          ...
          this.showNotification('Notificaciones activadas', 'Recibirás notificaciones cuando lleguen nuevos mensajes');
        } else {
    ...
    showNotification(title, body, onClick = null) {
      ...
      // Reproducir sonido
      try {
        this.notificationSound.play().catch(err => console.log('Error al reproducir sonido:', err));
      } catch (e) {
      ...
      const notification = new Notification(title, {
        body: body,
        icon: '/favicons/apple-touch-icon.png'  // Actualizado para usar tu favicon existente
      });
      ...
      try {
        ...
        this.socket.on('message', (message) => {
          ...
          if (!exists) {
            ...
            // Mostrar notificación personalizada si el mensaje es de otra persona y la ventana no está activa
            if (message.senderId !== this.userId && !this.windowActive) {
              let notificationTitle = '';

              // Personalizar el mensaje según el remitente
              if (message.sender === 'Mau') {
                notificationTitle = 'Max te ha mandado nuevo mensaje';
              } else if (message.sender === 'Ari') {
                notificationTitle = 'Ari te ha mandado mensaje';
              } else {
                notificationTitle = `${message.sender || this.partnerName || 'Tu contacto'} te ha mandado un mensaje`;
              }
